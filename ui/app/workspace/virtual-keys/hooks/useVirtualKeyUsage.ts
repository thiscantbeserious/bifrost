 ...
 *
 * Note: APs are only fetched for the first assigned user (hooks can't run in a loop). For multi-
 * user keys where a later assignee holds the managing AP, this won't find it — we'd need a
 * dedicated "AP by VK" backend endpoint to handle that cleanly.
 */
